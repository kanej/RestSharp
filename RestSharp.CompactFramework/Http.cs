...
namespace RestSharp
{
    ...
    public partial class Http : IHttp, IHttpFactory
    {
        ...
        private void AppendCookies(HttpWebRequest webRequest)
        {
            ...
            var cookiesAsKeyValues = this.Cookies.Select(c => c.Name + "=" + c.Value).ToArray();
            var cookieHeaderText = "Cookie: " + string.Join("; ", cookiesAsKeyValues);
            webRequest.Headers.Add(cookieHeaderText);
            this.CookieContainer.AddRange(this.Cookies.Select(c => new Cookie() { Name = c.Name, Value = c.Value }));
        ...
        private static void ExtractResponseData(HttpResponse response, HttpWebResponse webResponse)
        {
            using (webResponse)
            {
                ...
                var cookies = webResponse.Headers.AllKeys.Where(k => k == "Set-Cookie");
                if (cookies.Any())
                {
